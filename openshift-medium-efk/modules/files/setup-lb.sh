...
# OpenShift lb. It runs on host startup.
...
curl -sOL https://repo.toast.sh/helper/slack.sh | \
 bash -s -- --token=TATRUQ6P2/BAY9WSD7C/1bCckidSMB8KctWf2CgbHGtN Start: \`lb\` $(hostname)
...
[/var/log/messages]
log_stream_name = openshift-lb-{instance_id}
