...
//  value = "${length(var.node_eip) < 2 ? element(aws_eip.node1.*.public_ip, 0) : var.node_eip[0]}"
//}
...
//  value = "${length(var.node_eip) < 2 ? element(aws_eip.node2.*.public_ip, 0) : var.node_eip[1]}"
//}
